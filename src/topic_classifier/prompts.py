...
#
# 2026-07-31: 이벤트 쪽과 같은 결함 — 구간 정의가 이 가이드와 각 프롬프트 하단 "점수 기준"에
# 두 벌 있었고 경계가 어긋나 있었다(두 번째 구간 시작이 여기서는 0.70, 하단에서는 0.75).
# 하필 TOPIC_ASSIGN_SCORE_THRESHOLD 가 0.75 라 어느 눈금을 따르느냐로 배정이 갈린다.
# 구간 정의는 하단 "점수 기준" 한 곳(레이어마다 의미가 다르므로 그쪽이 단일 출처)으로 모으고,
# 여기서는 값을 어떻게 고를지만 지시한다.
# create 를 골랐을 때의 척도 의미를 명시하는 것도 이벤트 쪽과 같은 이유다 — 그쪽 실측에서
# create 인데 score 0.95 를 준 건이 20건이었다. 자세한 근거는 event_classifier/prompts.py 주석 참고.
SCORE_PLACEHOLDER = "<0.00~1.00>"

_SCORE_GUIDE = f"""score 는 "이 이벤트가 가장 가까운 후보 토픽에 속한다"는 확신도입니다.
create 를 고를 때도 이 의미는 그대로입니다 — 후보에 속하지 않는다고 판단해 create 한다면
score 는 낮아야 합니다. 자기 판단에 대한 확신도가 아닙니다.
위 {SCORE_PLACEHOLDER} 는 형식 표시일 뿐이니 그대로 쓰지 말고, 아래 "점수 기준"에 따라 매기세요."""
...
def build_parent_topic_assignment_prompt(
    ...
    build_topic_assignment_prompt(개별 사건 연속성 기준)와 달리, 여러 사건을 인과관계·
    동일 이슈·동일 일정·동일 사회현상으로 묶는 "하나의 사회적 이슈" 단위로 판단한다
    (docs/entity_definitions.md "토픽" 정의 기준). 여러 이슈를 뭉뚱그린 넓은 도메인·
    정책영역 버킷은 토픽이 아니다 — 그렇게 정의하면 서로 무관한 사건들이 과병합된다.
    """
...
상위(부모) 토픽이란:
- 하나의 사회적 이슈를 구성하는 연관된 사건들을 포괄하는 단위입니다. 여러 사건이 인과관계, 동일 이슈, 동일 일정, 동일 사회적 현상으로 묶일 때 하나의 토픽입니다.
- "한미 통상 협력", "노동정책 논의"처럼 여러 이슈를 아우르는 넓은 도메인·정책영역·관계축이 아닙니다. 그런 이름은 사실상 뉴스 카테고리이며 토픽이 아닙니다.
- 네이밍 테스트: 토픽명을 이벤트명 앞에 "<토픽명> — <이벤트명>" 형태로 붙였을 때, 하나의 사건 전체 이름처럼 자연스럽게 읽혀야 합니다. 예: "2026년 6.3 지방선거 — 사전투표 부실 논란"은 자연스럽지만, "정치 — 사전투표 부실 논란"이나 "선거 제도 — 사전투표 부실 논란"처럼 카테고리·도메인이 앞에 오면 부자연스럽습니다.

판단 규칙:
- 대상 이벤트가 후보 상위 토픽과 같은 하나의 사회적 이슈(같은 인과관계·같은 일정·같은 사회적 현상)에 속하면, 구체적 사건이 서로 달라도 assign 하세요.
- 후보 상위 토픽이 여러 이슈를 뭉뚱그린 넓은 도메인이거나 카테고리명이면 그 후보로 assign 하지 말고, 이 이벤트에 맞는 더 구체적인 이슈로 create 하세요.
- 후보 상위 토픽 중 이 이벤트가 속하는 사회적 이슈를 담는 것이 하나도 없을 때만 create 하세요.
- assign하는 경우 topic_id는 반드시 후보 토픽에 존재하는 값만 사용하세요.
- create하는 경우 new_title은 하나의 사회적 이슈명으로 15~30자 이내로 작성하세요. 여러 도메인을 아우르는 넓은 주제명이 아니라, 네이밍 테스트를 통과하는 구체적 이슈명이어야 합니다.
- new_title을 "정치", "경제", "사회", "국제"처럼 일반 뉴스 카테고리명 단독으로 짓지 마세요.
- new_title을 이 이벤트 하나만을 가리키는 단일 사건명으로 짓지 마세요 — 향후 관련 후속 사건들을 함께 담을 수 있는 이슈 단위여야 합니다.
- new_title은 문장이 아니라 뉴스 분류 카테고리처럼 읽혀야 하며, "~했다", "~됐다", "~한다" 같은 종결형과 마침표를 쓰지 마세요.
- assign 시 reason에는 "무관", "관련성이 없", "연관성이 없", "새로운 사건", "새로운 사안" 같은 표현을 쓰지 마세요. 대신 어떤 사회적 이슈에 속하는지 한 문장으로 설명하세요.
- reason은 판단 근거를 한 문장으로 간결하게 작성하세요.
- 마크다운 코드블록을 사용하지 말고 JSON 객체만 반환하세요.
- 위 필드 외 추가 필드를 포함하지 마세요.

점수 기준:
- 0.90~1.00: 같은 사회적 이슈임이 확실
- 0.75~0.89: 같은 이슈로 볼 수 있으나 일부 불확실
- 0.40~0.74: 인접 이슈이나 별도 이슈로 분리 가능
- 0.00~0.39: 전혀 다른 이슈
"""
...
- new_title을 "첫째 날", "둘째 날", "N일차", "N주차"처럼 단순 시간 구분으로 짓지 마세요 — 서브토픽은 시간이 아니라 쟁점 축으로 나눕니다.
- new_title을 "찬성", "반대"처럼 기사 찬반·논조 속성으로 짓지 마세요 — 서브토픽은 기사 속성이 아니라 사건들이 속하는 쟁점 갈래입니다.
